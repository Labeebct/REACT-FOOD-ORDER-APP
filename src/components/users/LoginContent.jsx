...
function LoginContent() {
  return (
   <main className='w-full h-[calc(100vh-4.5rem)] flex items-center justify-center'>
     <section className='w-[33%] xl:w-[66%] min-w-[370px] h-[34rem] bg-white overflow-hidden rounded-[.6rem] flex justify-center'>
        <div className="hidden xl:flex w-1/2 h-full ">
            <img src={loginLeftImg} alt="foodimage" className='h-full w-full' />
        </div>
               <form className="w-1/2 text-black min-w-[310px] h-full flex  flex-col items-center px-[4.4rem] py-9 gap-4 flex-1">
                  <h4 className='text-[2.2rem] mt-2 font-playfair'>Login</h4>
                  <div className='w-full h-auto flex flex-col mt-10 gap-2'>
                    <label htmlFor="email" className='font-roboto pl-2 text-[gray] text-[.8rem]'>Email</label>
                    <input spellCheck={false} type="text" className='h-[2.5rem] border rounded-[3rem] outline-none px-5 text-[.9rem] border-[#3939392d] drop-shadow-sm' />
                    <label htmlFor="email" className='font-roboto mt-3 pl-2 text-[gray] text-[.8rem]'>Password</label>
                    <input spellCheck={false} type="password" className='h-[2.5rem] border rounded-[3rem] outline-none px-5 text-[.9rem] border-[#3939392d] drop-shadow-sm' />
                  </div>
