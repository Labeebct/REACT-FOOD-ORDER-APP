...
function LoginContent() {
  ...
  const handleChange = (e) => {
    ...
    if(name == 'email'){
      ...
      setPassword(value)
      setPasswordLabel(!passwordRegex.test(value) ? 'Password should contains min 8 character' : '')
    }
      ...
      try {
        ...
        if(error.response){
          ...
          if(status == 422 || status == 401 || status == 404){
              ...
              setMessage(data.msg)
              setTimeout(() => Navigate(`/otp-verification/${data.email}`) , 800);
          } else {
      ...
      <section className="w-[33%] mb-2 xl:w-[66%] min-w-[370px] h-[34rem] bg-white overflow-hidden rounded-[.6rem] flex justify-center">
        ...
        <form onSubmit={handleSubmit} className="w-1/2 text-black min-w-[310px] h-full flex  flex-col items-center px-[4.6rem] py-9 gap-4 flex-1">
          ...
          <div className="w-full h-auto flex flex-col mt-9 gap-2">
            <label
              htmlFor="email"
              className={`font-roboto pl-2 ${ emailLabel && 'text-red-600' } text-[gray] text-[.75rem]`}
            >
            ...
            <label
              htmlFor="email"
              className={`font-roboto pl-2 ${ passwordLabel && 'text-red-600' } text-[gray] text-[.75rem]`}
            >
          ...
          <p className={`text-center ${loginSuccess ? 'text-green-600' : 'text-red-500'} text-[.9rem]`}>{message}</p>
          <button type="submit" className="px-8 py-2 mt-4 hover:bg-[#FF5A00] font-poppins font-[700] tracking-wider active:scale-[.96] ease-out duration-100 top-[21rem] right-24 bg-[#FF7A00] rounded-3xl text-white cursor-pointer">
            LOGIN
          </button>
        </form>
      </section>
